...
const LOCAL_DISCORD_PRESENCE_BASE = 'http://127.0.0.1:5678'; // Required for lyrics forwarding
...
const fetchFromGithub = (video_id) => {
  ...
  // Dynamic.lrc format:
  // [00:00.56]<00:00.56>char<00:00.68>based...
  ...
  return (async () => {
    ...
    // 1) Prioritize Dynamic.lrc (char-timed) first
...
if (req.type === 'GET_CLOUD_STATE') {
  ...
  if (req.type === 'SYNC_HISTORY') {
    ...
    const fetchSharedJson = async (payload) => {
      const tryFetch = async (url, init, label) => {
        ...
        try {
          ...
          // data remains null for non-JSON responses
        }
        if (!res.ok) {
      ...
      // 1) Try both URLs as JSON (handles environments where trailing slash causes redirect)
      ...
      // 2) If still failing, try form submission to avoid preflight (works if server supports it)
      //    * Do not set Content-Type to application/json (simple request)
      const formBody = new URLSearchParams();
      if (Array.isArray(payload.text)) {
        // Batch via form submission is spec-undefined, fall through to individual
    ...
    const translateViaShared = async () => {
      ...
      // Removed individual request fallback (pMap); use batch-only sending
      try {
        // Send all lines at once
        const data = await fetchSharedJson({ text: texts, target_lang: target });

        // Pattern 1: { text: ["translation1", "translation2"...] } format
        ...
        // Pattern 2: { translations: [{text: "translation1"}, ...] } format
        ...
        // Pattern 3: Single string (e.g. when request was a single line)
        ...
        // Throw error on batch failure to trigger DeepL fallback
    ...
    (async () => {
      try {
        ...
        // Auto-fallback to DeepL if shared translation fails and DeepL key is available
  ...
  if (req.type === 'GET_LYRICS') {
    ...
    (async () => {
      ...
      // Respond once only (do not wait for the slower one)
      let responded = false;
      ...
      // Push LRCHub candidates/config/requests so UI can be updated even after GitHub responds first
      ...
      // Parallel execution (stop waiting for both)
      let hubRes = null;
      let gitRes = null;
      ...
      const handleHub = async () => {
        ...
        // Not yet responded & LRCHub has lyrics → respond immediately
        ...
        // Already responded via GitHub → notify only candidates/config as late update
      ...
      const handleGit = async () => {
        ...
        if (
          ...
          // candidates/config/requests will be updated via pushMetaUpdate if LRCHub comes later
          sendOnce({
            success: true,
            lyrics: d.lyrics,
            dynamicLines: d.dynamicLines || null,
      ...
      // Done if either side responded
      if (responded) return;

      // Reaching here means neither source had lyrics. Return candidates if available
      let sharedCandidates = [];
      let sharedConfig = null;
      let sharedRequests = [];
  ...
  if (req.type === 'GET_TRANSLATION') {
    ...
    (async () => {
      ...
      // 1) Try GitHub translation/<lang>.txt first
      if (vid) {
      ...
      // 2) Fall back to LRCHub API only for missing ones
      if (remaining.length) {
        try {
      ...
      // Add langs missing from both GitHub and API to missing
      reqLangs.forEach(l => {
