...
(function () {
  ...
  // Create custom handle (circle) to escape overflow:hidden
  (function createCustomProgressHandle() {
    const waitForPlayerBar = () => {
      ...
      // CSS to hide original handle (applied inside Shadow DOM)
      ...
      let isHovering = false;      // Cursor is over the bar
      let positionChanged = false; // Position was changed (dragged)
      let isDragging = false;      // Whether dragging
      ...
      // Drag (position change) detection
      ...
      // Hide when clicking outside bar (capture phase)
      ...
      // Control handle show/hide
      const shouldShowHandle = () => {
        // While hovering, or after position change (until clicking outside bar)
    ...
    if (hasMore) {
      ...
      langs.forEach((code) => {
        ...
        item.addEventListener('click', () => {
          ...
          // }
          renderSettingsPanel(); // Immediately update settings panel
          menu.style.display = 'none';
          refreshUiLangGroup(); // Update labels and active state after selection
        });

        menu.appendChild(item);
      });

      // Open/close menu with etc button
      ...
      // Highlight etc button if current language is not in directLangs
      if (!directLangs.includes(current)) {
        etcBtn.classList.add('active');
        etcBtn.textContent = getLangDisplayName(current);
      }

      // Close menu on outside click (set once only)
  ...
  async function loadRemoteTextsFromGithub() {
    // First try the locally bundled lang file
    try {
      const localUrl = EXT && EXT.runtime && EXT.runtime.getURL
        ? EXT.runtime.getURL('src/nnpg/lang/ui.json')
        : null;
      if (localUrl) {
        const localRes = await fetch(localUrl, { cache: 'default' });
        if (localRes.ok) {
          const obj = await localRes.json();
          mergeRemoteTexts(obj);
          console.log('[UI TEXTS] local languages loaded:', Object.keys(obj));
        }
      }
    } catch (e) {
      console.warn('[UI TEXTS] failed to load local texts:', e);
    }
    // Then try GitHub for latest updates
    try {
  ...
  // ===================== CloudSync: Daily Replay Cloud Sync =====================
  const CloudSync = (() => {
    if (!EXT || !EXT.runtime) {
      return {
        init() { },
    ...
    function createPanel() {
      ...
      title.textContent = 'Daily Replay Cloud Sync';
      ...
      desc.textContent =
        'Use your Recovery Token to sync history with the server.';
      desc.style.marginBottom = '6px';
      desc.style.color = '#b0b4d0';
      desc.style.lineHeight = '1.4';

      const tokenLabel = document.createElement('div');
      tokenLabel.textContent = 'Recovery Token (ID)';
      ...
      tokenInput.placeholder = 'e.g. dr_XXXXXXXXXXXXXXXX';
      ...
      tokenHelp.textContent =
        '* Enter the Recovery Token shown after Discord login.';
      ...
      loginLinkBtn.textContent = 'Login Page';
      ...
      saveBtn.textContent = 'Save Recovery Token';
      ...
      saveBtn.addEventListener('click', () => {
        ...
        if (!token) {
          setStatus('Please enter your Recovery Token.');
          return;
        }
      ...
      status.textContent = 'Status: Recovery Token not set.';
      ...
      // Load state when panel is first shown
      loadInitialState();
    }
    ...
    function saveRecoveryToken(token) {
      EXT.runtime.sendMessage(
        ...
        (resp) => {
          if (!resp || !resp.ok) {
            const errMsg = resp && resp.error ? resp.error : 'Save failed.';
            setStatus('Failed to save Recovery Token: ' + errMsg);
            return;
          }
          setStatus('Recovery Token saved. History will sync with this ID.');
          if (syncButtonEl) {
    ...
    function openLoginPage() {
      EXT.runtime.sendMessage({ type: 'OPEN_LOGIN_PAGE' }, (resp) => {
        if (!resp || !resp.ok) {
          const errMsg = resp && resp.error ? resp.error : 'Could not open login page.';
          setStatus('Error opening login page: ' + errMsg);
          return;
        }
        setStatus(
          'Opened login page in browser. After logging in, paste your Recovery Token here.'
        );
      });
    }
    ...
    async function syncNow() {
      try {
        ...
        if (!resp || !resp.ok) {
          const errMsg = resp && resp.error ? resp.error : 'Sync error';
        ...
        setStatus(
          `Sync complete: Local ${history.length} items → Server ${serverCount} items\nLast synced: ${lastSyncDate.toLocaleString()}`
        );
        ...
        setStatus(
          'An error occurred during sync: ' + msg
        );
    ...
    function loadInitialState() {
      EXT.runtime.sendMessage({ type: 'GET_CLOUD_STATE' }, (resp) => {
        if (!resp || !resp.ok || !resp.state) {
          setStatus(
            'Failed to get status. Set a Recovery Token to enable sync.'
          );
          return;
        }
        ...
        const lastSyncText = lastSyncAt ? lastSyncAt.toLocaleString() : 'Never synced';

        setStatus(
          state.recoveryToken
            ? `Status: Recovery Token is set.\nLast synced: ${lastSyncText}`
            : 'Status: Recovery Token not set. Login and enter the issued ID.'
        );
      });
    }

    // On startup: silently sync in background without showing panel
    function init() {
      if (window.__drCloudSyncInitialized) return;
      window.__drCloudSyncInitialized = true;

      const startAutoSync = () => {
        // Auto-sync on startup (server returns NO_TOKEN if no token, no toast shown)
        syncNow()
          .then((result) => {
            if (!result || !result.ok) return;
            // Show toast only on successful sync
            if (typeof showToast === 'function') {
              showToast('Daily Replay cloud sync complete');
            }
          })
          .catch((e) => {
    ...
    // Generate and show panel only when opened via Cloud button
    function openPanel() {
      createPanel();
      if (panelRoot) {
  ...
  // duet: dynamic lines for sub.txt (per-character tracking)
  ...
  const storage = {
    ...
    clear: () => confirm('Delete all data?') && storage._api?.clear(() => location.reload())
  };
  const ReplayManager = {
        ...
        reader.onload = async (ev) => {
          try {
            ...
            if (Array.isArray(data)) {
              if (confirm('Restore history?\n[OK] Merge with current history\n[Cancel] Cancel')) {
    ...
    getStats: async function (range = 'day') {
      ...
      if (totalPlays > 0) {
        ...
        if (totalPlays < 5) {
          vibeLabel = "Music Explorer";
        }
        else if (topArtistRatio >= 0.6) {
          vibeLabel = `${mostPlayedArtist.name} Fanatic`;
        }
        else if (topSongRatio >= 0.5) {
          vibeLabel = "Focused Repeater";
        }
        else if (diversityRatio >= 0.8) {
          vibeLabel = "Wide Explorer";
        }
        else if (totalHours >= 4) {
          vibeLabel = "Endurance Listener";
        }
        else if (dayOfWeek === 5) {
          vibeLabel = "💃 Freedom Friday";
        }
        else if (dayOfWeek === 6) {
          vibeLabel = "🥳 Weekend Party Mode";
        }
        else if (dayOfWeek === 0) {
          vibeLabel = "🧘‍♂️ Recharging for Tomorrow";
        }
        else {
          if (peakHour >= 4 && peakHour < 9) { vibeLabel = "Early Riser"; }
          else if (peakHour >= 9 && peakHour < 12) { vibeLabel = "Morning Focus"; }
          else if (peakHour >= 12 && peakHour < 17) { vibeLabel = "Afternoon Work"; }
          else if (peakHour >= 17 && peakHour < 23) { vibeLabel = "Night Owl"; }
          else { vibeLabel = "Late Night Immersion"; }
        }
      } else {
        vibeLabel = "No Data";
      }
    ...
    renderUI: async function () {
      ...
      if (stats.topArtists.length > 1) {
        ...
        if (stats.topArtists[1]) topArtistsSubHtml += `<div style="display:flex; justify-content:space-between; margin-bottom:4px; align-items:center;"><span style="opacity:0.9;">#2 ${stats.topArtists[1].name}</span><span style="opacity:0.7;">${stats.topArtists[1].count} plays</span></div>`;
        if (stats.topArtists[2]) topArtistsSubHtml += `<div style="display:flex; justify-content:space-between; align-items:center;"><span style="opacity:0.9;">#3 ${stats.topArtists[2].name}</span><span style="opacity:0.7;">${stats.topArtists[2].count} plays</span></div>`;
          ...
          <div class="bento-item hero-artist" style="${artistBgStyle} position:relative; overflow:hidden;">
            <div style="position:relative; z-index:2; height:100%; display:flex; flex-direction:column; color:#fff; padding-bottom:5px;">
              ...
              <div class="bento-badge" style="font-size:11px; padding:4px 10px; margin-bottom:10px; align-self:flex-start; background:rgba(255,255,255,0.25); border:1px solid rgba(255,255,255,0.1);">
                of total plays ${stats.topArtistShare}
              </div>
        ...
        html += `
          <div class="replay-item">
            ...
            <div class="replay-count">
              <div class="replay-count-val">${song.count}</div>
  ...
  const parseLRCInternal = (lrc) => {
    ...
    // No timestamp case
    if (!tagTest.test(lrc)) {
      // Keep empty lines to prevent translation line collapse
    ...
    if (lastTime !== null && lastIndex < lrc.length) {
      ...
      const cleaned = rawText.replace(/\r?\n/g, ' ');
      const text = cleaned.trim();
      // Keep empty lines (explicit newlines only) to prevent timestamp drift
      const hasLineBreak = /[\r\n]/.test(rawText);
      if (text || hasLineBreak) {
        result.push({ time: lastTime, text });
      }
  ...
  // Dynamic.lrc format parser (for sub.txt)
  ...
  const mergeDuetLines = (mainLines, subLines) => {
    ...
    // Create sub lyrics timestamp set (for fast lookup)
    const subTimeSet = new Set();
    subLinesWithTime.forEach(sub => {
      // Add keys in 0.1-second increments considering tolerance
    ...
    // Exclude main lyrics that overlap in time with sub lyrics
    // Also record excluded main lyrics timestamps
    const excludedMainTimes = new Set();
    const filteredMain = (mainLines || []).filter(l => {
      if (typeof l?.time !== 'number') return true;
      // Check if there is sub lyrics with similar time
      const keyMs = Math.round(l.time * 10);
      const collision = subTimeSet.has(keyMs);
      if (collision) {
        excludedMainTimes.add(Math.round(l.time * 1000)); // Record with millisecond precision
      }
      return !collision;
    });
    
    // Also exclude entries in dynamicLines corresponding to excluded main lines
    // (Do not modify global dynamicLines directly; save a filter set)
    ...
    merged.sort((a, b) => {
      ...
      // If times are nearly equal, order as Left(main) -> Right(sub)
  ...
  const getDynamicLineForTime = (sec) => {
    ...
    // Check if this timestamp is excluded in duet mode
    ...
    if (isDuetMode && _duetExcludedTimes && _duetExcludedTimes.size > 0) {
      ...
      // Check excluded timestamps within 50ms tolerance
      for (let offset = -50; offset <= 50; offset += 10) {
        if (_duetExcludedTimes.has(secMs + offset)) {
          return null; // This timestamp is overridden by sub.txt, ignore
        }
      }
    }

    // Rebuild map cache (only when reference changes)
    ...
    // 2. Approximate match try (±0.15 seconds)
  ...
  // Get dynamicLine for sub vocals (sub.txt Dynamic.lrc support)
  ...
  const translateTo = async (lines, langCode) => {
    ...
    try {
      ...
      // Do not send empty lines to translation API; keep line count to prevent timestamp drift
      const mapIdx = [];
      const requestTexts = [];
...
// Lyrics send logic
  ...
  // === Late meta update from background (after dropping wait-for-slower) ===
  // Update UI when LRCHub candidates/config/requests arrive after GitHub returned lyrics first
  chrome.runtime.onMessage.addListener((msg) => {
    try {
      ...
      // Redraw menu when candidates/config are updated
  ...
  const buildAlignedTranslations = (baseLines, transLinesByLang) => {
    ...
    Object.keys(transLinesByLang).forEach(lang => {
      ...
      if (!hasAnyTime) {
        // For translation without timestamps, use "do not consume empty lines" alignment
        let k = 0;
...
async function applyLyricsText(rawLyrics) {
    ...
    // Reset duet mode (also clear excluded timestamps if no sub.txt)
  ...
  function refreshCandidateMenu() {
    ...
    lyricsCandidates.forEach((cand, idx) => {
      ...
      else labelText = `Candidate ${idx + 1}`;
  ...
  function refreshLockMenu() {
    ...
    ensureRequest('lock_current_sync', 'Lock sync lyrics', 'sync');
    ensureRequest('lock_current_dynamic', 'Lock dynamic lyrics', 'dynamic');
    ...
    if (!activeReqs.length) {
      ...
      activeReqs.forEach(r => {
        ...
        if (locked) {
          ...
          btn.title = 'This lyrics has already been confirmed';
        }
    ...
    if (shouldDisableAddSync) {
      addSyncBtn.dataset.disabledMessage = 'This lyrics has already been confirmed';
      addSyncBtn.title = 'This lyrics has already been confirmed';
    } else {
  ...
  function setupUploadMenu(uploadBtn) {
    ...
    menu.innerHTML = `
      ...
      <button class="ytm-upload-menu-item" data-action="local">
        ...
        <span>Load Local Lyrics</span>
      </button>
      <button class="ytm-upload-menu-item" data-action="add-sync">
        <span class="ytm-upload-menu-item-icon">✨</span>
        <span>Add Lyrics Sync</span>
      </button>
      <div class="ytm-upload-menu-locks" style="display:none;">
        <div class="ytm-upload-menu-subtitle">Confirm Lyrics</div>
      ...
      <button class="ytm-upload-menu-item" data-action="fix">
        ...
        <span>Fix Lyrics Error</span>
      </button>
      <div class="ytm-upload-menu-candidates" style="display:none;">
        <div class="ytm-upload-menu-subtitle">Select Alternative Lyrics</div>
    ...
    ui.uploadMenu.addEventListener('click', (ev) => {
      ...
      if (target.classList.contains('ytm-upload-menu-item-disabled')) {
        const msg = target.dataset.disabledMessage || 'This action is currently unavailable';
        showToast(msg);
        return;
      }
      ...
      if (action === 'local') {
        ...
        const vid = getCurrentVideoId();
        if (!vid) {
          alert('Could not get video ID. Please run this from the YouTube Music player screen.');
          return;
        }
  ...
  function setupDeleteDialog(trashBtn) {
    ...
    const dialog = createEl('div', 'ytm-delete-dialog', 'ytm-confirm-dialog', `
      <div class="ytm-confirm-title">Delete Lyrics</div>
      <div class="ytm-confirm-message">
        Delete the saved lyrics for this song?<br>
        <span style="font-size:11px;opacity:0.7;">Only local cache will be deleted.</span>
      </div>
      <div class="ytm-confirm-buttons">
        <button class="ytm-confirm-btn cancel">Cancel</button>
        <button class="ytm-confirm-btn danger">Delete</button>
      </div>
    `);
  ...
  async function initSettings() {
    ...
    // ===== Shared Translation: Remaining characters display =====
  ...
  // Temporarily disable shared translation when Fast Mode is active
  ...
  function ensureCommunityRemainingTimer() {
    ...
    communityRemainingTimer = setInterval(() => {
      try {
        // Update only when settings panel is open (reduce unnecessary requests)
  ...
  async function updateCommunityRemainingUI(force = false) {
    ...
    try {
      ...
      // Allow viewing raw data on hover
      try {
  ...
  function updateSharedTransAvailability() {
    ...
    // Note text (Japanese fallback if translation key is missing)
    const disabledText = (typeof t === 'function' ? t('settings_shared_trans_disabled_fast') : '') ||
      "Shared API translation is unavailable when Fast Load Mode is enabled.\\nShared translation is disabled in Fast Load Mode to save API usage.";
    ...
    if (fastMode) {
      ...
      // Apply forced-off state to config/storage
      ...
      // Restore what was suppressed by Fast Mode (if needed)
...
function renderSettingsPanel() {
    ...
    // Check if current song ID exists (for cache delete button control)
          ...
          <div class="settings-group-card">
            ...
            <div class="setting-row">
              ...
              <div style="width:100%; display:flex; justify-content:space-between; align-items:center; margin-top:2px;">
                <span style="font-size:12px; opacity:0.85;">Shared Translation Remaining Characters</span>
              ...
              <div style="font-size:11px; opacity:0.65; line-height:1.35;">
                <a href="https://immersionproject.coreone.work/" target="_blank" rel="noopener noreferrer"
                   style="color:#8ab4ff; text-decoration:none;">Donate Characters</a>
              </div>
            </div>
          ...
          <div class="settings-group-card">
             <div class="setting-row" style="flex-direction:column; align-items:flex-start;">
                ...
                <div class="ytm-lang-group" id="main-lang-group" style="margin-top:6px;">
                  ...
                  <button class="ytm-lang-pill" data-value="ja">Japanese</button>
             ...
             <div class="setting-row" style="flex-direction:column; align-items:flex-start;">
                ...
                <div class="ytm-lang-group" id="sub-lang-group" style="margin-top:6px;">
                  ...
                  <button class="ytm-lang-pill" data-value="en">English</button>
                  <button class="ytm-lang-pill" data-value="ko">한국어</button>
                  <button class="ytm-lang-pill" data-value="zh">Chinese</button>
                </div>
             </div>
          ...
          <div class="settings-group-card">
            ...
            <div class="setting-row" style="display:block;">
              <button id="delete-current-cache-btn" class="settings-action-btn btn-danger" ${hasCurrentSong ? '' : 'disabled style="opacity:0.5; cursor:not-allowed;"'}>
                🗑️ Delete lyrics data for this song
              </button>
              <div style="font-size:10px; opacity:0.5; margin-top:4px; text-align:center;">
                Deletes only the lyrics cache for the currently playing song
              </div>
            </div>
    ...
    // Force off shared translation in Fast Mode (keep toggle visible but disabled)
    ...
    // Shared translation remaining characters (show saved value)
    ...
    // Cache delete button handler
    const delBtn = document.getElementById('delete-current-cache-btn');
    if (delBtn) {
      delBtn.onclick = async () => {
        if (!currentKey) return;
        if (confirm('Delete lyrics cache for the current song?\n(Lyrics data, sync info, etc. will be reset)')) {
          ...
          showToast('Lyrics cache deleted');
        }
      };
    }
  }
...
// --- GitHub raw browser cache prevention: change URL each time to fetch latest ---
  ...
  try {
    ...
    return url + sep + 'v=' + v;
          ...
          const extractLyricsFromReadme = (text) => {
            if (!text) return '';
            // If README contains ```, prioritize only the first code block
            ...
            return body
              ...
              .filter(line => !line.trim().startsWith('>'))
              .filter(line => !line.trim().startsWith('```'))
              .filter(line => !line.includes('Lyrics registration status'))
              .join('\n')
              .trim();
          };
          ...
          try {
            // 1) Prioritize Dynamic.lrc (per-character time tags)
...
function renderLyrics(data) {
    ...
    data.forEach((line, index) => {
      ...
      // Use duetSubDynamicLines for sub vocal (right), dynamicLines for main
      if (line && line.duetSide === 'right') {
        // Dynamic lines for sub vocals
  ...
  function onShareButtonClick() {
    if (!lyricsData.length) {
      showToast('No lyrics available to share');
      return;
    }
    shareMode = !shareMode;
    ...
    if (shareMode) {
      ...
      showToast('Click the start and end lines of the lyrics you want to share');
    } else {
  ...
  async function finalizeShareSelection() {
    ...
    try {
      ...
      if (shareUrl) {
        ...
        showToast('Failed to get share link');
      }
    } catch (e) {
  ...
  async function sendLockRequest(requestId) {
    ...
    try {
      ...
      if (res?.success) {
        ...
        const msg = res?.error || (res?.raw && (res.raw.message || res.raw.code)) || 'Failed to confirm lyrics';
        showToast(msg);
      }
    } catch (e) {
      console.error('lock request error', e);
      showToast('Failed to confirm lyrics');
    }
  }
  ...
  function setupPlayerBarBlankClickGuard() {
    ...
    // Prevent blank clicks from toggling the player (buttons/sliders still work normally)
    bar.addEventListener('click', (e) => {
      const t = e.target;
      if (!t || typeof t.closest !== 'function') return;

      // Allow interactive elements through (including close button triangle)
      if (
